...
if [ -f ./releases ]; then
   ...
   im=$(curl https://api.github.com/repos/subspace/subspace/releases | jq '[0].assets | map(select(.name | startswith("subspace-cli-ubuntu-x86_64"))) | .[0].browser_download_url')
   echo $im
   fi
